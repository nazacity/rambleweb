...
const BlogContent = () => {
  ...
  const handleState = async () => {
    ...
    try {
      const res = await postSocial(
        `/api/employees/changeblogcategorystate/${data._id}`,
        {
  ...
  const fetchBlogCategories = async () => {
    dispatch(setLoading(true));
    try {
      const res = await getSocial('/api/everyone/blogcategories');
  ...
  const onSubmit = async (data) => {
    try {
      dispatch(setLoading(true));
      const res = await postSocial('/api/employees/createblogcategory', data);
  ...
  return (
    ...
    >
      ...
      <DeleteDialog
        ...
        title={'ต้องการเปลี่ยนสถานะ Blog Category นี้ใช่หรือไม่'}
      />
      <BlogDialog
        open={blogOpen}
